    ...
    const fetchPackageDetails = async () => {
      ...
      try {
        const response = await axios.get(
          `https://mjtrainingcamp.my.id/api/v1/package/get?id=${idPackage}`,
      ...
      try {
        ...
        if (packageDetails.value.type === 'Membership') {
          response = await axios.post(
            'https://mjtrainingcamp.my.id/api/v1/membership/buy',
            formData,
            {
              headers: {
          ...
          response = await axios.post(
            'https://mjtrainingcamp.my.id/api/v1/visitpackage/buy',
            formData,
            {
              headers: {
                Authorization: `Bearer ${token}`,
