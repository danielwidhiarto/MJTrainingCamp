    ...
    async addTrainer() {
      ...
      try {
        const response = await axios.post(
          'https://mjtrainingcamp.my.id/api/v1/trainer/add',
          this.currentTrainer,
          {
            headers: {
    ...
    async updateTrainer() {
      ...
      try {
        const response = await axios.put(
          `https://mjtrainingcamp.my.id/api/v1/trainer/update/${this.currentTrainer.idTrainer}`,
          this.currentTrainer,
          {
            headers: {
              'Content-Type': 'application/json',
    ...
    async deleteTrainer(idTrainer) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          try {
            await axios.delete(
              `https://mjtrainingcamp.my.id/api/v1/trainer/delete/${idTrainer}`,
              {
                headers: {
                  Authorization: `Bearer ${this.token}`,
                },
