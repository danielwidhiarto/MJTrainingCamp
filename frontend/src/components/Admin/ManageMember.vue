  ...
  setup() {
    ...
    const fetchMembers = async () => {
      try {
        ...
        const response = await axios.get(
          'http://ec2-13-229-129-198.ap-southeast-1.compute.amazonaws.com/api/v1/member/getall',
          {
    ...
    const viewMember = async idMember => {
      ...
      try {
        const token = localStorage.getItem('token')
        const response = await axios.get(
          `http://ec2-13-229-129-198.ap-southeast-1.compute.amazonaws.com/api/v1/member/get?id=${idMember}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        )
    ...
    const deleteMember = async idMember => {
      try {
        const token = localStorage.getItem('token')
        await axios.delete(
          `http://ec2-13-229-129-198.ap-southeast-1.compute.amazonaws.com/api/v1/member/delete/${idMember}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        )
